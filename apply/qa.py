...
def _company_match_key(value: object) -> str:
    ...
    words = [
        ...
        if word not in ordinary_suffixes
    ]
    return "".join(words)
...
def _company_matches(company: object, question: str, company_context: str = "") -> bool:
    ...
    match_key = _company_match_key(company)
    context_match_key = _company_match_key(company_context)
    if not key:
        return False
    # When the caller knows which employer owns the form, require an exact
    # normalized match.  Substring matching leaked facts between distinct firms
    # such as Crowe/Crowell & Moring and Sentry/Sentry Insurance.
    if context_key == key:
        return True
    if context_match_key and context_match_key == match_key:
        return True
    ...
    # names such as "Meta" from matching ordinary words such as "metadata".
    words = re.findall(r"[a-z0-9]+", str(company or "").lower())
    if not words:
        return False
    pattern = r"(?<![a-z0-9])" + r"[^a-z0-9]+".join(map(re.escape, words)) + r"(?![a-z0-9])"
    return re.search(pattern, str(question or "").lower()) is not None
...
HARD_BLOCKED_PATTERNS = [
    ...
    r"\b(previously interviewed|interviewed (?:at|with|for)|applied (?:to|with)|previously applied|prior application|previous application)\b",
...
def _no_clearance_value(value: object) -> bool:
    text = re.sub(r"[^a-z0-9]+", " ", str(value or "").strip().lower()).strip()
    return text in {"none", "no clearance", "no security clearance"}
...
def _institution_option(institution: str, options: list[str]) -> str | None:
    """Match a known school without fuzzy-merging it into another institution.

    School menus often add a location suffix, while the approved profile may
    already include one.  A token-prefix match safely handles those suffixes,
    but unlike the generic 0.6-overlap matcher it cannot turn Plano West Senior
    High School into Plano Senior High School.  Ambiguous prefixes fail closed.
    """
    expected = re.findall(r"[a-z0-9]+", str(institution or "").casefold())
    if not expected:
        return None
    matches = []
    for option in options:
        candidate = re.findall(r"[a-z0-9]+", str(option).casefold())
        if not candidate:
            continue
        shorter = min(len(expected), len(candidate))
        if expected[:shorter] == candidate[:shorter]:
...
def _approved_high_school_answer(approved: dict, options: list[str]) -> str | None:
    ...
    if not options:
        return school
    direct = _institution_option(school, options)
    if direct:
        return direct
...
def _approved_college_answer(options: list[str]) -> str | None:
    ...
    return _institution_option(school, options) or school
...
def explicit_approved_answers(controls: list[dict], key_field: str = "id_or_name",
    ...
    for original in controls:
        ...
        if answer is not None:
            ...
            if expected:
                candidates = [expected]
                if _no_clearance_value(expected):
                    candidates.extend(["No clearance", "None"])
                answer = _first_matching_option(candidates, options) if options else expected
        ...
        elif (re.search(r"\bgovernment (?:entity|agency|employment)\b", question)
        ...
        elif re.search(r"\binternship\b", question) and re.search(r"\bstart(?: date)?\b", question):
            summer_2027 = availability.get("summer_2027") or {}
            answer = summer_2027.get("start")
            ...
            if isinstance(expected, bool):
                answer = _render_boolean(expected, options)
        elif re.search(r"\b(applied (?:to|with)|previously applied|prior application|previous application)\b", question):
            expected = _company_fact_first(
            ...
            if publications is False or publications == []:
                answer = _render_boolean(False, options) if options else "None"
...
def _blocked_answer_is_approved(control: dict, answer: object, approved: dict) -> bool:
    ...
    if re.search(r"\b(security clearance|clearance level|public trust|secret clearance|top secret|ts/sci)\b", question):
        ...
        if _no_clearance_value(expected):
            candidates.extend(["No clearance", "None"])
        approved_answer = (_first_matching_option(candidates, options)
    ...
    if re.search(r"\binternship\b", question) and re.search(r"\bstart(?: date)?\b", question):
        summer_2027 = availability.get("summer_2027") or {}
        expected = str(summer_2027.get("start") or "").strip()
        return bool(expected and expected.casefold() == answer_text.strip().casefold())
    ...
    if re.search(r"\b(compensation|salary|pay (?:range|rate)|hourly rate|base pay|bonus|equity|expected (?:pay|salary)|desired (?:pay|salary)|(?:pay|compensation) expectations?)\b", question):
    ...
    if re.search(r"\b(applied (?:to|with)|previously applied|prior application|previous application)\b", question):
        expected = _company_fact_first(
            control, approved, "prior_application", "prior_interview_or_application",
        )
        return isinstance(expected, bool) and _answer_boolean(answer) is expected
    ...
    if re.search(r"\bpublications?\b", question):
        publications = (approved.get("professional") or {}).get("publications")
        if publications is False or publications == []:
            return _answer_boolean(answer_text) is False
        if isinstance(publications, list) and publications:
            expected = "; ".join(str(item) for item in publications)
            return expected.lower() == answer_text.strip().lower()
        return False
