...
# specific numbers; figures marked (~) are David-authorized estimates: he should
...
1. Owned end-to-end infrastructure for 3 production apps (patient, provider, admin): AWS + Supabase Postgres with RLS, S3 media storage, CI/CD, and monitoring, handling PHI-sensitive data across ~40 pilot patients and 9-stage video workflows
...
5. Built retry/fallback orchestration across 3 STT/LLM/TTS vendor pairs, raising call completion rate ~8pp and isolating provider outages without dropping live calls
6. Instrumented end-to-end latency tracing across the voice pipeline (SIP ingress -> STT -> LLM -> TTS), cutting p99 turn latency ~35% by eliminating tail spikes
...
10. jobhunt — Autonomous job-application pipeline: watchers diff 3 GitHub listing repos (~450 postings tracked), Claude tailors LaTeX resumes per JD, Playwright adapters submit across Greenhouse/Lever/Ashby/Workday with Gmail human-in-the-loop approvals (Python, Playwright, SQLite, launchd)
